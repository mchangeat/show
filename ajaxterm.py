...
class Multiplex:
	...
	def proc_write(self,fd,s):
		try:
			...
			self.proc_kill(fd)
	def dump(self,fd,color=1,html=True):
		try:
			if html:
				return self.proc[fd]['term'].dumphtml(color)
			else:
				return self.proc[fd]['term'].dumplatin1()
		except KeyError:
			return False
	def loop(self):
		while self.run():
...
class AjaxTerm:
	...
	def __call__(self, environ, start_response):
		...
		if req.PATH_INFO.endswith('/u'):
			...
			time.sleep(0.002)
			dump=self.multi.dump(term,c,True)
...
class LocalTerm:
	...
	def update(self, s, k, c, w, h):
		...
		if k:
			self.multi.proc_write(term,k)
		time.sleep(0.002)
		dump=self.multi.dump(term,c,False)
